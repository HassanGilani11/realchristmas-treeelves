...
function ProcessingAndMaxPeriod() {
 ...
 if(window.Shopify.shop == "lotusbakeries.myshopify.com" || window.Shopify.shop == "zipcode-validator-staging.myshopify.com"){
 if(max_delivery_period == 0){
 for(var i=0; i<blackout_days_array.length; i++)
 {
 const event = new Date(new_default_selected_date);
 if (processing_period != 0) {
 event.setDate(event.getDate() + 1);
